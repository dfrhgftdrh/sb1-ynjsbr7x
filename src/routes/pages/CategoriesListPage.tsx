...
export default function CategoriesListPage() {
  ...
  useEffect(() => {
    updateMetaTags({
      title: 'Browse Categories - RingBuz',
      description: 'Explore our curated categories of wallpapers and ringtones. Find the perfect content for your device organized by themes and styles.',
      keywords: 'categories, wallpaper categories, ringtone categories, browse content, themes',
      canonicalUrl: 'https://ringbuz.in/categories',
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {wallpaperCategories.map((category) => (
              <Link
                ...
                className="group relative overflow-hidden rounded-xl bg-gradient-to-br from-pink-500/10 to-pink-600/10 border border-pink-500/20 hover:border-pink-500/40 transition-all"
              >
                {/* Thumbnail */}
                <div className="aspect-video w-full overflow-hidden">
                  {category.thumbnail_url ? (
                    ...
                    <div className="w-full h-full bg-gradient-to-br from-pink-500/20 to-pink-600/20 flex items-center justify-center">
                      <ImageIcon size={48} className="text-pink-500/50" />
                    </div>
                  )}
                </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {ringtoneCategories.map((category) => (
              <Link
                key={category.id}
                to={`/explore/${category.slug}`}
                className="group relative overflow-hidden rounded-xl bg-gradient-to-br from-blue-500/10 to-blue-600/10 border border-blue-500/20 hover:border-blue-500/40 transition-all"
              >
                {/* Thumbnail */}
                <div className="aspect-video w-full overflow-hidden">
                  {category.thumbnail_url ? (
                    ...
                    <div className="w-full h-full bg-gradient-to-br from-blue-500/20 to-blue-600/20 flex items-center justify-center">
                      <Music size={48} className="text-blue-500/50" />
                    </div>
                  )}
                </div>
